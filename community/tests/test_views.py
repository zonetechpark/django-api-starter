...
class UpdateSinglePuppyTest(TestCase):
    ...
    def test_valid_update_puppy(self):
        response = client.put(reverse('puppy-detail', kwargs={'pk': self.muffin.pk}),
...
class DeleteSinglePuppyTest(TestCase):
    ...
    def test_valid_delete_puppy(self):
        response = client.delete(reverse('puppy-detail', kwargs={'pk': self.muffin.pk}))
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)

    def test_invalid_delete_puppy(self):
        response = client.delete(reverse('puppy-detail', kwargs={'pk': uuid.uuid4()}))
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
